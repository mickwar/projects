...
Let $C^+(S)$ be the space of non-negative real continuous functions on S, with S some compact subset of $\R^d$. Let $W$ be a stochastic process in $C^+(S)$ and $\omega_0$ a positive constant. When $W(s) = Y V(s)$ for all $s \in S$, for some $Y$ and $V=\{V(s)\}_{s\in S}$, satisfies:
\begin{enumerate}[(a)]
