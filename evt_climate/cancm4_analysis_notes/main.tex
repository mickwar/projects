...
% When each component in the quadruple $\{\m{F}_t, v_t, \m{G}_t, \m{W}_t\}$ is known, it is possible to obtain posterior distributions of $(\m{\theta}_t|y_1,\ldots,y_T)$ for $t<T$ (details omitted). 
% 
% For each time-series, we fit a dynamic linear model (DLM) with annual and semi-annual periods. 
% 
% From the DLMs we obtain a smoothed version of the time-series, and then take the difference between the original series with the smoothed version. The differences are called the anomalies and the subsequent analyses are performed on them. 
% 
